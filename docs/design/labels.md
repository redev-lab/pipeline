...
★**2026-06-11 t-전쟁 최종(수검 4/4 통과) — t = "동결(행위제한)이 시작된 최초 시점".**
의제처리 폴리곤(WHERE)에 **6소스 후보 중 earliest-genuine을 t로** 채택한다(소스
우선순위 아님). 보류 66%→**4%**(70/73 폴리곤, dissolve 51구역). OA 결정고시 이력
마이닝이 후보 다수를 대지만 **신통선정일이 더 이르면 신통이 채택**된다(7건) —
신통은 선정 즉시 토허·권리산정기준일·건축제한이 걸려 *동결이 거기서 시작*되므로
인과적으로 옳은 t(§4-1). 채택 안 된 다른 날짜는 `t_alt`에 보존.

| source(positive) | **t =** | t_source | 비고 |
|---|---|---|---|
| 의제처리(OA 회수) | 최초 결정고시일 | `oa_first_decision` | 사이클 인지 마이닝(완료고시 후 결정만). 재건축·후속단계 배제 |
| 의제처리(촉진 sub) | 부모 촉진지구 결정일 | `promotion_parent` | 근사치(sub 고유일 아님) |
| 의제처리(keyless 단일지정) | NTFC 고시일 | `ntfc_direct` | 조건: 제목 "지정"&¬"변경" + OA 더이른결정 없음. "변경"이면 보류(추정 금지) |
| 공공재개발 | 선정일 | `public_redev` | 1-b. t_kind=designation은 t_alt에 OA 교차검증 |
| 신통 | **신통선정일** | `shintong_select` | ★동등 earliest 후보 — 동결 시작=선정일(인과적 t). 7건 채택 |
| 정비사업 | 정비사업 고시일 | `first_designation_gosi` | 동등 earliest 후보(교차검증 겸) |
...
### 4-1. positive (label=1, certainty=positive) — WHERE=의제처리 폴리곤, WHEN=earliest-genuine
★**t = "동결(행위제한)이 시작된 최초 시점".** 의제처리 폴리곤이 WHERE(경계)+NTFC_SN을
준다. t는 6소스 후보(OA/촉진부모/ntfc_direct/공공재개발/신통/정비사업) 중 **최소 날짜**
(earliest-genuine). 소스 우선순위가 아니라 **인과적 최소**다.
- **신통선정일도 동등 earliest 후보.** t=지정고시일로 잡으면 선정~지정 사이 *동결된
  기간의 피처(노후도 등)에 선정의 결과가 새어* R1 위반 → 신통은 선정 즉시 토허·권리
  산정기준일·건축제한이 걸리므로 동결 시작=선정일이 **인과적으로 옳다**(보수적 선택 아님).
  '선정' vs '지정'은 t_source로 추적, 지정고시일은 t_alt에 보존.
- **사이클 가드**: OA 마이닝은 완료고시(관리처분/준공) *이후* 결정만 현재 사이클로
  (흑석2: 1985 완료 옛 사이클 배제→2008). **촉진 sub**는 부모 촉진지구 결정 근사.
- **t 안 붙는 폴리곤 = 보류(제외)** — §11. 추정 날짜로 안 채운다.
  (실측: 70/73 폴리곤 t 확보, 3개만 진짜 보류["변경"뿐] — §11.)
...
| 리스크 | 어디서 | 어떻게 |
|---|---|---|
| **R4** | `certainty` 컬럼 | ZoneTable(지정구역)=positive / {해제,신축파생}=reliable_negative / 노후미지정=uncertain. PU 학습기가 uncertain 차등. |
| **R5** | 자연 발생 | 같은 PNU가 해제(t1,neg)+ZoneTable지정(t2,pos) → 자동 두 행. 충돌해소는 *같은 (pnu,t)* 에서만. |
...
building_gis ─┬→ aging.노후도 as-of-t ─┬→ R2 오염 / 신축파생·노후미지정      │
              └──────────────────────┘                                  │
zone_boundary.load_zones() ─→ ZoneTable(폴리곤+t+t_source+t_alt) ─→ ∩parcels = positive
  (의제처리+OA마이닝+6소스 earliest-genuine, 수검통과)                       │
...
★ labels는 **parcels·building_gis·zone_boundary(ZoneTable)·해제**에 의존. positive의
t는 zone_boundary가 끝냄(labels는 ∩만). `transactions.py`(AVM)·`regulation.py`는
Phase 5. 배제레이어는 v1 보류(§11).
...
| `_positives_from_zonetable(zone_table, parcels)` | ★`zone_boundary.load_zones()`의 ZoneTable(폴리곤+t+t_source+t_alt) ∩ parcels 대표점 → positive rows. t-해소는 zone_boundary가 끝냄(여기선 ∩만). |
...
> 이미 끝냈다**(수검 통과). labels.py는 ZoneTable을 받아 ∩parcels만 한다 — t 재계산 금지.
> UQ-밖 standalone 신통/공공재개발(대표지번 seed)은 v1.1(§4.5 한계).
