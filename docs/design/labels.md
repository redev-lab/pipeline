...
- **★[v1.1 시점정합 — 용도지역 종상향 역류] 용도지역은 2026 스냅샷뿐.** v1.1에서 과거 라벨
  (positive 2001~2025)에 2026 용도지역을 피처로 붙이면 누수(R1): 2026엔 3종이어도 지정 *당시*
  2종이었을 수 있어 "종상향의 결과"를 "원인"으로 거꾸로 배운다. → v1.1은 **근사 수용 + 한계
  명시**(연도별 용도지역 이력 미보유), 또는 결정고시 이력으로 보정. (DATA_SOURCES.md 등록.)
- **★[v1.1 시점정합 — 역세권 개통연도] 역사마스터에 개통일 컬럼 없음**(2026-06 검수). 과거 라벨에
  "그때 없던 역"까지 거리계산하면 누수. → **노선 개통연도 표 보강**(소형, v1.1) 후 as-of-t 역거리.
  (공시지가는 연도별 2001~2026 입수라 이 문제 없음.)
...
- **★[근본 한계 — "요건 됐는데 무산"을 v1은 못 배운다] hard negative 24노드뿐.**
  Phase 3 실측(2026-06-11): 학습셋 음성 24,338 중 **신축파생 24,314(aging≈0, 정의상
  자명 분리) + 해제 24개뿐.** "노후 요건은 충족했으나 무산·해제된" 진짜 어려운 음성이
  24노드 → 모델이 *"낡았는데 왜 안 됐나"*(외생변수 R18: 주민동의·정치)를 사실상 학습
  불가. **평가에서 hard-neg 성적은 n=24라 통계적으로 무의미**(분리 리포트하되 그 수치로
  결론 금지). 이는 §6 예언 천장의 데이터적 실체. → **k-hop 해제 확장(§4.6)의 우선순위가
  이 발견으로 상승**(v1.1 1순위): 해제 seed에서 인접 그래프 확장 + 노후조건으로 hard-neg
  공급을 늘려야 "진단(요건)"과 "예언(무산)"의 분리가 데이터로 뒷받침된다.
- **★[데이터병목 3호 — infer 과대예측] R4 PU 갭이 추론에서 터짐.** Phase 6 실측(2026-06-12):
  production B1이 전 노드의 **81%를 양성**으로(중앙 확률 0.977). 학습 음성이 신축(aging≈0)뿐이라
  B1이 배운 건 "낡았나" → 4구 대부분 노후 → 거의 다 양성. uncertain(노후미지정 5.6만)을 양성과
  못 가름(그 대조를 학습에서 본 적 없음, R4). IoU 통제비교: B1넓은 0.294 > B0 0.129 > B1타이트
  0.019(★타이트가 더 나쁨 = 점수는 노후도지 지정 아님, 가장 老한 곳이 대부분 미지정 = R18 공간적
  실체). → 진짜 해법 = **PU 학습(uncertain 인지) 또는 가치/입지 피처(v1.1)**. 심장1·심장2와 같은 결.
