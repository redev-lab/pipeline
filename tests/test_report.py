...
def test_template_report_clean_sections():        # 재설계: 5절·태그/결론/한계 없음(화면이 카드·접힘 표시)
    f = _display_facts(_DATA)
    rep = _template_report(f)
    assert "### 될까" in rep and "### 얼마" in rep   # 머리 = '### 라벨'(화면 카드 분리용)
    assert "재개발" in rep                          # 요건판정(될까 절)
    assert "[" not in rep and "결론" not in rep and "한계" not in rep   # 태그·결론·한계 본문 제외
    assert "환경 점수" not in rep                    # 환경순위는 카드로(본문 중복 금지)
    assert verify_numbers(rep, f)["ok"]             # 환각 0
...
def test_noncandidate_report_hallucination_zero():  # 결함 5·6 통합(환각 0 유지)
    rep = generate_report(_NONCAND, complete_fn=lambda s, u: _template_report(_display_facts(_NONCAND)))
    assert rep["hallucination"]["ok"]
    assert "R15" not in rep["report_text"] and "39%" not in rep["report_text"]
    assert "source_facts" in rep                   # ★출처 메타 보존(화면 툴팁용)
